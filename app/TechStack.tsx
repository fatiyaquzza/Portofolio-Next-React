...
export default function TechStack() {
  ...
  const cursorRefs = useRef<(HTMLLIElement | null)[]>([]);
  ...
  useEffect(() => {
    ...
    const lines = lineRefs.current.filter(
      (line): line is HTMLLIElement => line !== null
    );
    const cursors = cursorRefs.current.filter(
      (cursor): cursor is HTMLLIElement => cursor !== null
    );
    ...
    const ctx = gsap.context(() => {
      ...
      blocks.forEach((block, groupIndex) => {
        ...
        const blockLines = stackGroups[groupIndex].items
          ...
          .filter((line): line is HTMLLIElement => line !== null);
  ...
  return (
    <section
      id="skills"
      ref={sectionRef}
      className="relative overflow-hidden bg-surface-page px-5 py-24 font-sans sm:px-8 md:px-16 md:py-28 lg:px-24 xl:px-32"
    >
      <div
      ...
      />
      <div
        className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-surface-section to-transparent"
        aria-hidden="true"
      />
      ...
      <div className="relative mx-auto max-w-[1400px]">
        ...
        >
          <div>
            ...
            <h2 className="mt-4 text-[clamp(2.5rem,5vw,5rem)] font-semibold leading-[1.02] text-foreground">
              Tools I use
            </h2>
          </div>
          <p className="max-w-xl text-sm leading-7 text-ink-caption md:text-right md:text-[15px]">
          ...
          {stackGroups.map((group, groupIndex) => (
            ...
            >
              <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <h3 className="text-[1.65rem] font-semibold leading-none text-foreground sm:text-[2rem]">
                  {group.title}
                </h3>
                <p className="font-mono text-xs text-theme-accent sm:text-sm">
                  {group.command}
                </p>
              </div>

              <div className="theme-shadow overflow-hidden rounded-2xl border border-[#8D78FF]/18 bg-surface-terminal/70 shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_24px_80px_rgba(7,5,20,0.28)] backdrop-blur-xl">
                <div
                  ref={(node) => {
                    panelHeaderRefs.current[groupIndex] = node;
                  }}
                  className="flex items-center justify-between gap-4 border-b border-contrast/[0.07] bg-surface-terminal-header/70 px-4 py-3 sm:px-5"
                >
                  <p className="font-mono text-xs text-theme-accent sm:text-sm">
                    {group.path}
                  </p>
                ...
                <ul className="flex flex-wrap content-start gap-x-4 gap-y-2 px-4 py-5 font-mono sm:gap-x-6 sm:px-5 sm:py-6">
                  {group.items.map(({ name, Icon, color, tone }, itemIndex) => (
                    <li
                      ...
                      className="group inline-flex min-h-9 items-center gap-2 rounded-md px-2 py-1.5 text-left"
                    >
                      <span className="text-xs text-ink-faint transition group-hover:text-theme-accent">
                        &gt;
                      </span>
                      <span className="inline-flex items-center gap-2">
                        <Icon
                          ...
                          aria-hidden
                        />
                        <span className={`skill-name text-sm leading-none ${tone}`}>
                          {name}
                        </span>
                      </span>
                    </li>
